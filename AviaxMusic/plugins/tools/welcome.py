...
from AviaxMusic.utils.daxx_ban import admin_filter
...
def welcomepic(pic, user, chatname, id, uname, brightness_factor=1.3):
    background = Image.open("AviaxMusic/assets/wel2.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp, brightness_factor=brightness_factor) 
    pfp = pfp.resize((575, 575))
    draw = ImageDraw.Draw(background)
    font = ImageFont.truetype('AviaxMusic/assets/font4.ttf', size=70)
    welcome_font = ImageFont.truetype('AviaxMusic/assets/font4.ttf', size=61)
...
async def greet_new_member(_, member: ChatMemberUpdated):
    ...
    if member.new_chat_member and not member.old_chat_member and member.new_chat_member.status != "kicked":
        ...
        try:
            async for photo in app.get_chat_photos(user.id,1): 
                pic = await app.download_media(photo.file_id, file_name=f'{user.id}.jpg')
        ...
        try:
            ...
            if not name:
                name = ""
            chatname = member.chat.title if member.chat.title else ""
            id = user.id if user.id else ""
            uname = user.username if user.username else ""

            welcomeimg = welcomepic(
                pic, name, chatname, id, uname
            )
